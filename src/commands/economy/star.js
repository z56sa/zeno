...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetUser && amount) {
      ...
      collector.on('collect', async i => {
        if (i.customId === 'confirm_pay') {
          ...
          const curSender = db.getUser(sender.id, guildId);
    ...
    const userData = db.getUser(userToQuery.id, guildId);
    ...
    let files = [];
    if (member) {
      const rankData = db.getUserRank(userToQuery.id, guildId) || { xp: 0, level: 0, rank: 1 };
  ...
  async executePrefix(message, args) {
    ...
    // 1. فحص إذا كان هناك تحويل (مثال: #c @user 500 أو #star 500 @user)
