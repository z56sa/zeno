...
const fetchUserData = (userId, guildId) => {
  ...
  return {
    ...user,
    coins: Number(user.coins ?? user.gold ?? 0),
    credits: Number(user.coins ?? user.gold ?? 0),
...
module.exports = {
  name: 'gold',
  description: 'عرض الرصيد وبطاقة البروفايل أو تحويل عملة gold COIN 🪙 بأسلوب zeno',
  aliases: ['c', 'stars', 'credits', 'credit', 'coin', 'coins', 'bal', 'balance', 'starcoin', 'ذهب', 'رصيد', 'فلوس', 'كريدت'],
  data: new SlashCommandBuilder()
    .setName('gold')
    .setDescription('عرض الرصيد أو تحويل عملة gold COIN 🪙')
    .addUserOption(opt =>
      opt.setName('user')
  ...
  async execute(interaction) {
    ...
    if (targetUser && amount) {
      ...
      if (senderBalance < amount) {
        return interaction.editReply({
          content: `:warning: | **${sender.username}**, رصيدك غير كافي! رصيدك الحالي هو **${senderBalance.toLocaleString()}** 🪙 gold Coin.`
        });
      }
      ...
      const replyMessage = await interaction.editReply({
        content: `🤔 | **${sender.username}**, هل أنت متأكد من رغبتك في تحويل **${amount.toLocaleString()}** 🪙 gold Coin إلى <@${targetUser.id}>؟\n*(الضريبة: ${tax.toLocaleString()} 🪙 - سيستلم العضو: ${finalAmount.toLocaleString()} 🪙)*`,
        components: [row]
      });
      ...
      collector.on('collect', async i => {
        if (i.customId === 'confirm_pay') {
          ...
          await i.update({
            content: `🏧 | **${sender.username}**, تم تحويل **${finalAmount.toLocaleString()}** 🪙 gold Coin إلى <@${targetUser.id}> بنجاح! :money_with_wings:`,
            components: []
          });
        } else {
    ...
    const replyContent = userToQuery.id === interaction.user.id
      ? `💳 | **${interaction.user.username}**, رصيد حسابك الحالي هو : **${formattedBal}** 🪙 **gold Coin**`
      : `💳 | رصيد حساب **${userToQuery.username}** هو : **${formattedBal}** 🪙 **gold Coin**`;
  ...
  async executePrefix(message, args) {
    ...
    if (recipient && amount) {
      ...
      const promptMsg = await message.reply({
        content: `🤔 | **${sender.username}**, هل أنت متأكد من رغبتك في تحويل **${amount.toLocaleString()}** 🪙 gold Coin إلى <@${recipient.id}>؟\n*(الضريبة: ${tax.toLocaleString()} 🪙 - سيستلم العضو: ${finalAmount.toLocaleString()} 🪙)*`,
        components: [row]
      });
      ...
      collector.on('collect', async i => {
        if (i.customId === 'confirm_pay_msg') {
          ...
          await i.update({
            content: `🏧 | **${sender.username}**, قام بتحويل **${finalAmount.toLocaleString()}** 🪙 gold Coin إلى <@${recipient.id}> بنجاح! :money_with_wings:`,
            components: []
          });
        } else {
          await i.update({ content: '❌ | تم إلغاء عملية التحويل.', components: [] });
    ...
    const replyContent = userToQuery.id === sender.id
      ? `💳 | **${sender.username}**, رصيد حسابك الحالي هو : **${formattedBal}** 🪙 **gold Coin**`
      : `💳 | رصيد حساب **${userToQuery.username}** هو : **${formattedBal}** 🪙 **gold Coin**`;
