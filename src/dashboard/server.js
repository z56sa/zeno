...
formFieldsHtml = `                    <div class="space-y-6 text-right" dir="rtl">
            ...
            formFieldsHtml = "﻿<div id=\"staffTrackerRoot\" class=\"space-y-6 text-right\" dir=\"rtl\">\n\n    <!-- Header -->\n    <div class=\"bg-[#12141f] border border-white/5 p-6 rounded-2xl flex items-center justify-between shadow-xl\">\n        <div class=\"flex items-center gap-5\">\n            <div class=\"text-center\">\n                <span id=\"staffTotalActions\" class=\"text-2xl font-black text-purple-400 font-mono\">0</span>\n                <span class=\"text-[10px] text-gray-400 block\">إجمالي الإجراءات</span>\n            </div>\n            <div class=\"text-center\">\n                <span id=\"staffActiveCount\" class=\"text-2xl font-black text-emerald-400 font-mono\">0</span>\n                <span class=\"text-[10px] text-gray-400 block\">مشرفون نشطون</span>\n            </div>\n            <div class=\"text-center\">\n                <span id=\"staffTicketsClosed\" class=\"text-2xl font-black text-amber-400 font-mono\">0</span>\n                <span class=\"text-[10px] text-gray-400 block\">تذاكر مغلقة</span>\n            </div>\n        </div>\n        <div class=\"flex items-center gap-3\">\n            <div class=\"text-right\">\n                <h4 class=\"font-black text-white text-base\">تتبع نشاط الإدارة</h4>\n                <p class=\"text-gray-400 text-xs mt-0.5\">إحصائيات دقيقة لأداء فريق الإشراف</p>\n            </div>\n            <div class=\"w-11 h-11 rounded-xl bg-amber-600/20 text-amber-400 flex items-center justify-center text-xl border border-amber-500/30\">👮</div>\n        </div>\n    </div>\n\n    <!-- Filter Tabs -->\n    <div class=\"flex items-center gap-2 justify-end\">\n        <button type=\"button\" onclick=\"window.staffFilter('month')\" id=\"sfMonth\" class=\"px-4 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer\">الشهر</button>\n        <button type=\"button\" onclick=\"window.staffFilter('week')\" id=\"sfWeek\" class=\"px-4 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer\">الأسبوع</button>\n        <button type=\"button\" onclick=\"window.staffFilter('today')\" id=\"sfToday\" class=\"px-4 py-1.5 rounded-xl text-xs font-bold bg-purple-600 text-white transition shadow cursor-pointer\">اليوم</button>\n    </div>\n\n    <!-- Leaderboard Cards -->\n    <div id=\"staffLeaderboard\" class=\"space-y-3\">\n        <div class=\"py-12 bg-[#12141f] border border-white/5 rounded-2xl text-center text-xs text-gray-500\">جارٍ تحميل بيانات الإدارة...</div>\n    </div>\n\n    <!-- Settings Row -->\n    <div class=\"grid grid-cols-1 lg:grid-cols-2 gap-5\">\n        <!-- Staff Role Setting -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl\">\n            <div class=\"flex items-center justify-end gap-2 mb-4\">\n                <h6 class=\"font-black text-white text-sm\">إعدادات نظام التتبع</h6>\n                <span class=\"text-lg\">⚙️</span>\n            </div>\n            <div class=\"space-y-3\">\n                <div class=\"flex items-center justify-between\">\n                    <label class=\"toggle\"><input type=\"checkbox\" id=\"staffActivityEnabled\" onchange=\"window.saveStaffSetting('staff_activity_enabled', this.checked)\"><span class=\"slider\"></span></label>\n                    <span class=\"text-xs text-gray-300\">تفعيل تتبع النشاط</span>\n                </div>\n                <div class=\"text-right\">\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">رتبة الإدارة</label>\n                    <select id=\"staffRoleSelect\" onchange=\"window.saveStaffSetting('staff_role', this.value)\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-purple-500 cursor-pointer\">\n                        <option value=\"\">-- اختر رتبة الإدارة --</option>\n                        ${guildRoles.map(r => `<option value=\"${r.id}\" ${settings.staff_role === r.id ? 'selected' : ''}>@${r.name}</option>`).join('')}\n                    </select>\n                </div>\n                <div class=\"text-right\">\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">قناة سجل النشاط</label>\n                    <select id=\"staffLogChannel\" onchange=\"window.saveStaffSetting('staff_log_channel', this.value)\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-purple-500 cursor-pointer\">\n                        <option value=\"\">-- اختر القناة --</option>\n                        ${guildChannels.filter(c=>c.type===0).map(c => `<option value=\"${c.id}\" ${settings.staff_log_channel === c.id ? 'selected' : ''}>📝 ${c.name}</option>`).join('')}\n                    </select>\n                </div>\n                <button type=\"button\" onclick=\"window.resetStaffStats()\" class=\"w-full py-2 bg-rose-950/40 hover:bg-rose-900/60 text-rose-400 border border-rose-800/40 rounded-xl text-xs font-bold transition mt-2\">🗑️ تصفير جميع الإحصائيات</button>\n            </div>\n        </div>\n\n        <!-- Goals -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl\">\n            <div class=\"flex items-center justify-between mb-4\">\n                <button type=\"button\" onclick=\"window.addStaffGoal()\" class=\"px-3 py-1.5 bg-purple-600/30 hover:bg-purple-600 text-purple-300 hover:text-white border border-purple-500/30 rounded-xl text-xs font-bold transition\">+ هدف جديد</button>\n                <div class=\"flex items-center gap-2\">\n                    <h6 class=\"font-black text-white text-sm\">الأهداف الشهرية</h6>\n                    <span class=\"text-lg\">🎯</span>\n                </div>\n            </div>\n            <div id=\"staffGoalsList\" class=\"space-y-2\">\n                <p class=\"text-xs text-gray-500 text-center py-4\">لا توجد أهداف مضافة</p>\n            </div>\n        </div>\n    </div>\n</div>\n\n<script>\n(function() {\n    var currentPeriod = 'today';\n\n    function renderLeaderboard(data) {\n        var container = document.getElementById('staffLeaderboard');\n        if (!container) return;\n        if (!data || !data.length) {\n            container.innerHTML = '<div class=\"py-12 bg-[#12141f] border border-white/5 rounded-2xl text-center text-xs text-gray-500\">لا توجد بيانات نشاط حتى الآن 👮</div>';\n            return;\n        }\n        var totalActions = 0, totalTickets = 0;\n        var html = data.map(function(s, idx) {\n            var total = (s.mod_actions||0) + (s.tickets_closed||0) + (s.warns_count||0);\n            totalActions += total;\n            totalTickets += (s.tickets_closed||0);\n            var medal = idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '#' + (idx+1);\n            return '<div class=\"bg-[#0b0d14] border border-white/5 p-4 rounded-2xl flex items-center justify-between hover:border-purple-500/30 transition\">' +\n                '<div class=\"flex items-center gap-3\">' +\n                    '<div class=\"text-center min-w-[60px]\">' +\n                        '<span class=\"text-xs font-black text-white block\">' + (s.mod_actions||0) + '</span><span class=\"text-[9px] text-gray-500\">إجراءات</span>' +\n                    '</div>' +\n                    '<div class=\"text-center min-w-[60px]\">' +\n                        '<span class=\"text-xs font-black text-emerald-400 block\">' + (s.tickets_closed||0) + '</span><span class=\"text-[9px] text-gray-500\">تذاكر</span>' +\n                    '</div>' +\n                    '<div class=\"text-center min-w-[60px]\">' +\n                        '<span class=\"text-xs font-black text-amber-400 block\">' + (s.warns_count||0) + '</span><span class=\"text-[9px] text-gray-500\">تحذيرات</span>' +\n                    '</div>' +\n                '</div>' +\n                '<div class=\"flex items-center gap-3\">' +\n                    '<div class=\"text-right\">' +\n                        '<p class=\"text-xs font-black text-white\">' + (s.username || 'مشرف غير معروف') + '</p>' +\n                        '<p class=\"text-[11px] text-purple-400 font-bold\">' + total + ' نقطة نشاط</p>' +\n                    '</div>' +\n                    '<span class=\"text-xl w-10 text-center\">' + medal + '</span>' +\n                '</div>' +\n            '</div>';\n        }).join('');\n        container.innerHTML = html;\n        var ta = document.getElementById('staffTotalActions');\n        var sc = document.getElementById('staffActiveCount');\n        var tc = document.getElementById('staffTicketsClosed');\n        if (ta) ta.textContent = totalActions;\n        if (sc) sc.textContent = data.length;\n        if (tc) tc.textContent = totalTickets;\n    }\n\n    function loadStaffData(period) {\n        var gId = window.location.pathname.split('/')[2];\n        if (!gId) return;\n        fetch('/api/guild/' + gId + '/staff-activity?period=' + period)\n            .then(function(r) { return r.json(); })\n            .then(function(d) { if (d.success) renderLeaderboard(d.data); })\n            .catch(function() {});\n    }\n\n    window.staffFilter = function(period) {\n        currentPeriod = period;\n        ['today','week','month'].forEach(function(p) {\n            var btn = document.getElementById('sf' + p.charAt(0).toUpperCase() + p.slice(1));\n            if (btn) btn.className = p === period\n                ? 'px-4 py-1.5 rounded-xl text-xs font-bold bg-purple-600 text-white transition shadow cursor-pointer'\n                : 'px-4 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer';\n        });\n        loadStaffData(period);\n    };\n\n    window.saveStaffSetting = function(key, value) {\n        var gId = window.location.pathname.split('/')[2];\n        if (!gId) return;\n        var body = {}; body[key] = typeof value === 'boolean' ? (value ? 1 : 0) : value;\n        fetch('/api/guild/' + gId + '/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(function() {});\n    };\n\n    window.resetStaffStats = function() {\n        if (!confirm('هل أنت متأكد من تصفير جميع إحصائيات الإدارة؟')) return;\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/reset-staff-stats', { method: 'POST' })\n            .then(function(r) { return r.json(); })\n            .then(function(d) { if (d.success) { loadStaffData(currentPeriod); alert('تم التصفير بنجاح'); } })\n            .catch(function() {});\n    };\n\n    window.addStaffGoal = function() {\n        var title = prompt('عنوان الهدف:');\n        if (!title) return;\n        var target = prompt('القيمة المستهدفة (رقم):');\n        if (!target || isNaN(target)) return;\n        var type = prompt('نوع الهدف: tickets / mod_actions / warns');\n        if (!type) return;\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/staff-goals', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: title, target_type: type, target_value: parseInt(target) }) })\n            .then(function(r) { return r.json(); })\n            .then(function(d) { if (d.success) loadStaffGoals(); })\n            .catch(function() {});\n    };\n\n    function loadStaffGoals() {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/staff-goals')\n            .then(function(r) { return r.json(); })\n            .then(function(d) {\n                var list = document.getElementById('staffGoalsList');\n                if (!list) return;\n                if (!d.data || !d.data.length) { list.innerHTML = '<p class=\"text-xs text-gray-500 text-center py-4\">لا توجد أهداف مضافة</p>'; return; }\n                list.innerHTML = d.data.map(function(g) {\n                    return '<div class=\"flex items-center justify-between bg-[#0b0d14] border border-white/5 rounded-xl px-3 py-2\">' +\n                        '<button type=\"button\" onclick=\"window.deleteGoal(' + g.id + ')\" class=\"text-red-400 hover:text-red-300 text-xs\">🗑️</button>' +\n                        '<div class=\"text-right\"><p class=\"text-xs font-bold text-white\">' + g.title + '</p><p class=\"text-[10px] text-gray-500\">الهدف: ' + g.target_value + ' ' + g.target_type + '</p></div>' +\n                    '</div>';\n                }).join('');\n            }).catch(function() {});\n    }\n\n    window.deleteGoal = function(id) {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/staff-goals/' + id, { method: 'DELETE' })\n            .then(function() { loadStaffGoals(); }).catch(function() {});\n    };\n\n    // تفعيل التبديل من الإعدادات\n    var saEnabled = document.getElementById('staffActivityEnabled');\n    if (saEnabled) saEnabled.checked = ${settings.staff_activity_enabled ? 'true' : 'false'};\n\n    loadStaffData('today');\n    loadStaffGoals();\n})();\n</script>";
        } else if (section === 'quran') {
            formFieldsHtml = "﻿<div id=\"quranRadioRoot\" class=\"space-y-6 text-right\" dir=\"rtl\">\n\n    <!-- Header -->\n    <div class=\"bg-gradient-to-r from-[#0f2615] via-[#12141f] to-[#12141f] border border-emerald-800/30 p-6 rounded-2xl flex items-center justify-between shadow-xl\">\n        <div class=\"flex items-center gap-4\">\n            <div id=\"radioNowPlaying\" class=\"flex items-center gap-2 bg-emerald-950/40 px-3 py-1.5 rounded-xl border border-emerald-800/30\">\n                <span class=\"w-2 h-2 rounded-full bg-emerald-400 animate-pulse\"></span>\n                <span class=\"text-xs text-emerald-300 font-bold\" id=\"radioStatusText\">لا يوجد بث نشط</span>\n            </div>\n        </div>\n        <div class=\"flex items-center gap-3\">\n            <div class=\"text-right\">\n                <h4 class=\"font-black text-white text-base\">القرآن الكريم والراديو</h4>\n                <p class=\"text-gray-400 text-xs mt-0.5\">بث القرآن والإذاعات الإسلامية في قنوات الصوت</p>\n            </div>\n            <div class=\"w-11 h-11 rounded-xl bg-emerald-600/20 text-emerald-400 flex items-center justify-center text-xl border border-emerald-500/30\">🕌</div>\n        </div>\n    </div>\n\n    <!-- Main Grid -->\n    <div class=\"grid grid-cols-1 lg:grid-cols-3 gap-5\">\n\n        <!-- Station Picker + Controls -->\n        <div class=\"lg:col-span-1 space-y-4\">\n            <!-- Now Playing Card -->\n            <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl\">\n                <div class=\"flex items-center justify-end gap-2 mb-4\">\n                    <h6 class=\"font-black text-white text-sm\">التحكم بالبث</h6>\n                    <span>🎙️</span>\n                </div>\n                <div class=\"space-y-3\">\n                    <div>\n                        <label class=\"text-[11px] text-gray-400 block mb-1\">القناة الصوتية</label>\n                        <select id=\"radioVoiceChannel\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-emerald-500 cursor-pointer\">\n                            <option value=\"\">-- اختر القناة الصوتية --</option>\n                            ${guildChannels.filter(c=>c.type===2).map(c => `<option value=\"${c.id}\" ${settings.radio_voice_channel === c.id ? 'selected' : ''}>🔊 ${c.name}</option>`).join('')}\n                        </select>\n                    </div>\n                    <div>\n                        <label class=\"text-[11px] text-gray-400 block mb-1\">المحطة الإذاعية</label>\n                        <select id=\"radioStationSelect\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-emerald-500 cursor-pointer\">\n                            <option value=\"quran_makkah\">🕌 قرآن كريم - مكة المكرمة</option>\n                            <option value=\"quran_egypt\">📖 إذاعة القرآن الكريم - مصر</option>\n                            <option value=\"sunnah\">☀️ راديو السنة النبوية</option>\n                            <option value=\"madinah\">🌙 إذاعة المدينة المنورة</option>\n                            <option value=\"quran_tafseer\">📚 قرآن التفسير والعلوم</option>\n                        </select>\n                    </div>\n                    <div class=\"flex gap-2 mt-2\">\n                        <button type=\"button\" onclick=\"window.stopRadio()\" class=\"flex-1 py-2.5 bg-rose-950/40 hover:bg-rose-900/60 text-rose-400 border border-rose-800/40 rounded-xl text-xs font-bold transition\">⏹️ إيقاف</button>\n                        <button type=\"button\" onclick=\"window.startRadio()\" class=\"flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black transition shadow-lg\">▶️ تشغيل</button>\n                    </div>\n                </div>\n            </div>\n\n            <!-- Auto-start Setting -->\n            <div class=\"bg-[#12141f] border border-white/5 p-4 rounded-2xl shadow-xl\">\n                <div class=\"flex items-center justify-between mb-3\">\n                    <label class=\"toggle\"><input type=\"checkbox\" id=\"radioEnabled\" onchange=\"window.saveRadioSetting('radio_enabled', this.checked)\" ${settings.radio_enabled ? 'checked' : ''}><span class=\"slider\"></span></label>\n                    <span class=\"text-xs text-gray-300 font-bold\">تشغيل تلقائي عند بدء البوت</span>\n                </div>\n                <p class=\"text-[10px] text-gray-500 text-right\">عند التفعيل، سيبدأ البوت البث تلقائياً في القناة المختارة</p>\n            </div>\n        </div>\n\n        <!-- Stations List -->\n        <div class=\"lg:col-span-2 space-y-3\">\n            <div class=\"flex items-center justify-end gap-2 mb-2\">\n                <h6 class=\"font-black text-white text-sm\">المحطات المتاحة</h6>\n                <span>📻</span>\n            </div>\n\n            <div onclick=\"window.selectStation('quran_makkah')\" class=\"radio-station-card bg-[#12141f] border border-white/5 p-4 rounded-2xl flex items-center justify-between cursor-pointer hover:border-emerald-500/40 transition group\">\n                <div class=\"flex items-center gap-3\">\n                    <div class=\"flex flex-col items-end gap-0.5\">\n                        <span class=\"text-xs font-black text-white group-hover:text-emerald-300 transition\">قرآن كريم - مكة المكرمة</span>\n                        <span class=\"text-[10px] text-gray-500\">بث حي مباشر من مكة المكرمة</span>\n                    </div>\n                </div>\n                <div class=\"flex items-center gap-3\">\n                    <button type=\"button\" onclick=\"event.stopPropagation(); window.quickPlay('quran_makkah')\" class=\"px-3 py-1.5 bg-emerald-600/30 hover:bg-emerald-600 text-emerald-400 hover:text-white rounded-xl text-xs font-bold transition\">▶️ تشغيل</button>\n                    <div class=\"w-10 h-10 rounded-xl bg-emerald-950/40 border border-emerald-800/20 flex items-center justify-center text-xl\">🕌</div>\n                </div>\n            </div>\n\n            <div onclick=\"window.selectStation('quran_egypt')\" class=\"radio-station-card bg-[#12141f] border border-white/5 p-4 rounded-2xl flex items-center justify-between cursor-pointer hover:border-emerald-500/40 transition group\">\n                <div class=\"flex items-center gap-3\">\n                    <div class=\"flex flex-col items-end gap-0.5\">\n                        <span class=\"text-xs font-black text-white group-hover:text-emerald-300 transition\">إذاعة القرآن الكريم - مصر</span>\n                        <span class=\"text-[10px] text-gray-500\">إذاعة القرآن الكريم المصرية الرسمية</span>\n                    </div>\n                </div>\n                <div class=\"flex items-center gap-3\">\n                    <button type=\"button\" onclick=\"event.stopPropagation(); window.quickPlay('quran_egypt')\" class=\"px-3 py-1.5 bg-emerald-600/30 hover:bg-emerald-600 text-emerald-400 hover:text-white rounded-xl text-xs font-bold transition\">▶️ تشغيل</button>\n                    <div class=\"w-10 h-10 rounded-xl bg-emerald-950/40 border border-emerald-800/20 flex items-center justify-center text-xl\">📖</div>\n                </div>\n            </div>\n\n            <div onclick=\"window.selectStation('sunnah')\" class=\"radio-station-card bg-[#12141f] border border-white/5 p-4 rounded-2xl flex items-center justify-between cursor-pointer hover:border-amber-500/40 transition group\">\n                <div class=\"flex items-center gap-3\">\n                    <div class=\"flex flex-col items-end gap-0.5\">\n                        <span class=\"text-xs font-black text-white group-hover:text-amber-300 transition\">راديو السنة النبوية</span>\n                        <span class=\"text-[10px] text-gray-500\">أحاديث نبوية وعلوم السنة المطهرة</span>\n                    </div>\n                </div>\n                <div class=\"flex items-center gap-3\">\n                    <button type=\"button\" onclick=\"event.stopPropagation(); window.quickPlay('sunnah')\" class=\"px-3 py-1.5 bg-amber-600/30 hover:bg-amber-600 text-amber-400 hover:text-white rounded-xl text-xs font-bold transition\">▶️ تشغيل</button>\n                    <div class=\"w-10 h-10 rounded-xl bg-amber-950/40 border border-amber-800/20 flex items-center justify-center text-xl\">☀️</div>\n                </div>\n            </div>\n\n            <div onclick=\"window.selectStation('madinah')\" class=\"radio-station-card bg-[#12141f] border border-white/5 p-4 rounded-2xl flex items-center justify-between cursor-pointer hover:border-blue-500/40 transition group\">\n                <div class=\"flex items-center gap-3\">\n                    <div class=\"flex flex-col items-end gap-0.5\">\n                        <span class=\"text-xs font-black text-white group-hover:text-blue-300 transition\">إذاعة المدينة المنورة</span>\n                        <span class=\"text-[10px] text-gray-500\">بث من المدينة المنورة المباركة</span>\n                    </div>\n                </div>\n                <div class=\"flex items-center gap-3\">\n                    <button type=\"button\" onclick=\"event.stopPropagation(); window.quickPlay('madinah')\" class=\"px-3 py-1.5 bg-blue-600/30 hover:bg-blue-600 text-blue-400 hover:text-white rounded-xl text-xs font-bold transition\">▶️ تشغيل</button>\n                    <div class=\"w-10 h-10 rounded-xl bg-blue-950/40 border border-blue-800/20 flex items-center justify-center text-xl\">🌙</div>\n                </div>\n            </div>\n\n            <div onclick=\"window.selectStation('quran_tafseer')\" class=\"radio-station-card bg-[#12141f] border border-white/5 p-4 rounded-2xl flex items-center justify-between cursor-pointer hover:border-purple-500/40 transition group\">\n                <div class=\"flex items-center gap-3\">\n                    <div class=\"flex flex-col items-end gap-0.5\">\n                        <span class=\"text-xs font-black text-white group-hover:text-purple-300 transition\">قرآن التفسير والعلوم</span>\n                        <span class=\"text-[10px] text-gray-500\">تفسير القرآن والعلوم الشرعية</span>\n                    </div>\n                </div>\n                <div class=\"flex items-center gap-3\">\n                    <button type=\"button\" onclick=\"event.stopPropagation(); window.quickPlay('quran_tafseer')\" class=\"px-3 py-1.5 bg-purple-600/30 hover:bg-purple-600 text-purple-400 hover:text-white rounded-xl text-xs font-bold transition\">▶️ تشغيل</button>\n                    <div class=\"w-10 h-10 rounded-xl bg-purple-950/40 border border-purple-800/20 flex items-center justify-center text-xl\">📚</div>\n                </div>\n            </div>\n        </div>\n    </div>\n</div>\n\n<script>\n(function() {\n    var stationNames = {\n        quran_makkah: \"قرآن كريم - مكة المكرمة\",\n        quran_egypt: \"إذاعة القرآن الكريم - مصر\",\n        sunnah: \"راديو السنة النبوية\",\n        madinah: \"إذاعة المدينة المنورة\",\n        quran_tafseer: \"قرآن التفسير والعلوم\"\n    };\n\n    window.selectStation = function(key) {\n        var sel = document.getElementById('radioStationSelect');\n        if (sel) sel.value = key;\n    };\n\n    window.startRadio = function() {\n        var gId = window.location.pathname.split('/')[2];\n        var channel = document.getElementById('radioVoiceChannel').value;\n        var station = document.getElementById('radioStationSelect').value;\n        if (!channel) return alert('يرجى اختيار قناة صوتية أولاً');\n        fetch('/api/guild/' + gId + '/radio/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ channel_id: channel, station: station }) })\n            .then(function(r) { return r.json(); })\n            .then(function(d) {\n                var statusEl = document.getElementById('radioStatusText');\n                if (d.success) {\n                    if (statusEl) statusEl.textContent = 'يعمل الآن: ' + (stationNames[station] || station);\n                    window.saveRadioSetting('radio_voice_channel', channel);\n                    window.saveRadioSetting('radio_station', station);\n                } else alert('خطأ: ' + (d.error || 'فشل التشغيل'));\n            }).catch(function() { alert('خطأ في الاتصال بالسيرفر'); });\n    };\n\n    window.stopRadio = function() {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/radio/stop', { method: 'POST' })\n            .then(function(r) { return r.json(); })\n            .then(function(d) {\n                if (d.success) {\n                    var statusEl = document.getElementById('radioStatusText');\n                    if (statusEl) statusEl.textContent = 'لا يوجد بث نشط';\n                }\n            }).catch(function() {});\n    };\n\n    window.quickPlay = function(station) {\n        var sel = document.getElementById('radioStationSelect');\n        if (sel) sel.value = station;\n        window.startRadio();\n    };\n\n    window.saveRadioSetting = function(key, value) {\n        var gId = window.location.pathname.split('/')[2];\n        var body = {}; body[key] = typeof value === 'boolean' ? (value ? 1 : 0) : value;\n        fetch('/api/guild/' + gId + '/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(function() {});\n    };\n\n    // Check current status\n    (function checkStatus() {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/radio/status')\n            .then(function(r) { return r.json(); })\n            .then(function(d) {\n                var statusEl = document.getElementById('radioStatusText');\n                if (statusEl) statusEl.textContent = d.playing ? ('يعمل الآن: ' + (stationNames[d.station] || d.station)) : 'لا يوجد بث نشط';\n            }).catch(function() {});\n    })();\n})();\n</script>";
        } else if (section === 'applications') {
            formFieldsHtml = "﻿<div id=\"applicationsRoot\" class=\"space-y-6 text-right\" dir=\"rtl\">\n\n    <!-- Header -->\n    <div class=\"bg-[#12141f] border border-white/5 p-6 rounded-2xl flex items-center justify-between shadow-xl\">\n        <div class=\"flex items-center gap-5\">\n            <div class=\"text-center\"><span id=\"appTotalCount\" class=\"text-2xl font-black text-purple-400 font-mono\">0</span><span class=\"text-[10px] text-gray-400 block\">إجمالي الطلبات</span></div>\n            <div class=\"text-center\"><span id=\"appPendingCount\" class=\"text-2xl font-black text-amber-400 font-mono\">0</span><span class=\"text-[10px] text-gray-400 block\">قيد الانتظار</span></div>\n            <div class=\"text-center\"><span id=\"appAcceptedCount\" class=\"text-2xl font-black text-emerald-400 font-mono\">0</span><span class=\"text-[10px] text-gray-400 block\">مقبول</span></div>\n        </div>\n        <div class=\"flex items-center gap-3\">\n            <div class=\"text-right\">\n                <h4 class=\"font-black text-white text-base\">نظام التقديمات والتوظيف</h4>\n                <p class=\"text-gray-400 text-xs mt-0.5\">إدارة طلبات التقديم والتوظيف بالسيرفر</p>\n            </div>\n            <div class=\"w-11 h-11 rounded-xl bg-blue-600/20 text-blue-400 flex items-center justify-center text-xl border border-blue-500/30\">📝</div>\n        </div>\n    </div>\n\n    <!-- Main Grid -->\n    <div class=\"grid grid-cols-1 lg:grid-cols-5 gap-5\">\n\n        <!-- Create Application Form -->\n        <div class=\"lg:col-span-2 bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl space-y-4\">\n            <div class=\"flex items-center justify-end gap-2 border-b border-white/5 pb-3\">\n                <h6 class=\"font-black text-white text-sm\">إنشاء تقديم جديد</h6>\n                <span class=\"text-lg\">✨</span>\n            </div>\n            <div class=\"space-y-3\">\n                <div>\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">عنوان الوظيفة</label>\n                    <input type=\"text\" id=\"appTitle\" placeholder=\"مثال: تقديم على رتبة مشرف...\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none text-right\">\n                </div>\n                <div>\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">الوصف</label>\n                    <textarea id=\"appDesc\" rows=\"3\" placeholder=\"شرح مختصر عن الوظيفة...\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none text-right resize-none\"></textarea>\n                </div>\n                <div>\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">الأسئلة (سطر لكل سؤال، الحد 5)</label>\n                    <textarea id=\"appQuestions\" rows=\"5\" placeholder=\"ما عمرك؟&#10;كم ساعة يمكنك أن تكون نشطاً؟&#10;لماذا تريد الانضمام للإدارة؟\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none text-right resize-none\"></textarea>\n                </div>\n                <div>\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">قناة استلام الطلبات</label>\n                    <select id=\"appLogChannel\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-purple-500 cursor-pointer\">\n                        <option value=\"\">-- اختر القناة --</option>\n                        ${guildChannels.filter(c=>c.type===0).map(c => `<option value=\"${c.id}\">📝 ${c.name}</option>`).join('')}\n                    </select>\n                </div>\n                <div>\n                    <label class=\"text-[11px] text-gray-400 block mb-1\">رتبة القبول (اختياري)</label>\n                    <select id=\"appAcceptRole\" class=\"w-full bg-[#0b0d14] border border-white/5 text-xs text-white rounded-xl px-3 py-2 outline-none focus:border-purple-500 cursor-pointer\">\n                        <option value=\"\">-- بدون رتبة --</option>\n                        ${guildRoles.map(r => `<option value=\"${r.id}\">@${r.name}</option>`).join('')}\n                    </select>\n                </div>\n                <button type=\"button\" onclick=\"window.createApplication()\" class=\"w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-black transition shadow-lg mt-2\">✨ إنشاء التقديم</button>\n            </div>\n        </div>\n\n        <!-- Submissions List -->\n        <div class=\"lg:col-span-3 space-y-4\">\n            <!-- Filter Tabs -->\n            <div class=\"flex items-center gap-2 justify-end\">\n                <button type=\"button\" onclick=\"window.appFilter('rejected')\" id=\"afRejected\" class=\"px-3 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer\">مرفوض</button>\n                <button type=\"button\" onclick=\"window.appFilter('accepted')\" id=\"afAccepted\" class=\"px-3 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer\">مقبول</button>\n                <button type=\"button\" onclick=\"window.appFilter('pending')\" id=\"afPending\" class=\"px-3 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer\">قيد الانتظار</button>\n                <button type=\"button\" onclick=\"window.appFilter('all')\" id=\"afAll\" class=\"px-3 py-1.5 rounded-xl text-xs font-bold bg-purple-600 text-white transition shadow cursor-pointer\">الكل</button>\n            </div>\n            <!-- Submissions -->\n            <div id=\"submissionsList\" class=\"space-y-3\">\n                <div class=\"py-12 bg-[#12141f] border border-white/5 rounded-2xl text-center text-xs text-gray-500\">جارٍ تحميل الطلبات...</div>\n            </div>\n        </div>\n\n    </div>\n</div>\n\n<script>\n(function() {\n    var currentFilter = 'all';\n\n    function loadSubmissions(filter) {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/applications?status=' + filter)\n            .then(function(r) { return r.json(); })\n            .then(function(d) {\n                var list = document.getElementById('submissionsList');\n                if (!list) return;\n                var total = d.total || 0, pending = d.pending || 0, accepted = d.accepted || 0;\n                var tt = document.getElementById('appTotalCount');\n                var tp = document.getElementById('appPendingCount');\n                var ta = document.getElementById('appAcceptedCount');\n                if (tt) tt.textContent = total;\n                if (tp) tp.textContent = pending;\n                if (ta) ta.textContent = accepted;\n                if (!d.data || !d.data.length) {\n                    list.innerHTML = '<div class=\"py-12 bg-[#12141f] border border-white/5 rounded-2xl text-center text-xs text-gray-500\">لا توجد طلبات تقديم 📝</div>';\n                    return;\n                }\n                list.innerHTML = d.data.map(function(s) {\n                    var statusClass = s.status === 'accepted' ? 'text-emerald-400 bg-emerald-950/60' : s.status === 'rejected' ? 'text-rose-400 bg-rose-950/60' : 'text-amber-400 bg-amber-950/60';\n                    var statusText = s.status === 'accepted' ? 'مقبول ✅' : s.status === 'rejected' ? 'مرفوض ❌' : 'قيد الانتظار ⏳';\n                    var btns = s.status === 'pending' ? '<div class=\"flex items-center gap-2 mt-3\">' +\n                        '<button type=\"button\" onclick=\"window.reviewApp(' + s.id + \\',\"rejected\")\\' class=\"flex-1 py-1.5 bg-rose-950/40 hover:bg-rose-900 text-rose-400 hover:text-white border border-rose-800/40 rounded-xl text-xs font-bold transition\">❌ رفض</button>' +\n                        '<button type=\"button\" onclick=\"window.reviewApp(' + s.id + \\',\"accepted\")\\' class=\"flex-1 py-1.5 bg-emerald-950/40 hover:bg-emerald-900 text-emerald-400 hover:text-white border border-emerald-800/40 rounded-xl text-xs font-bold transition\">✅ قبول</button>' +\n                        '</div>' : '';\n                    var answers = [];\n                    try { answers = JSON.parse(s.answers); } catch(e) {}\n                    var answersHtml = answers.map(function(a, i) { return '<p class=\"text-[11px] text-gray-300 mt-1\"><span class=\"text-purple-400\">س' + (i+1) + ': </span>' + a + '</p>'; }).join('');\n                    return '<div class=\"bg-[#12141f] border border-white/5 p-4 rounded-2xl hover:border-purple-500/30 transition\">' +\n                        '<div class=\"flex items-center justify-between mb-2\">' +\n                            '<span class=\"text-[10px] font-bold px-2 py-0.5 rounded-lg ' + statusClass + '\">' + statusText + '</span>' +\n                            '<p class=\"text-xs font-black text-white\">' + (s.app_title || 'تقديم') + '</p>' +\n                        '</div>' +\n                        '<p class=\"text-[11px] text-gray-400 mb-2\">المتقدم: <span class=\"text-purple-300\">' + (s.username || s.user_id) + '</span></p>' +\n                        answersHtml + btns +\n                    '</div>';\n                }).join('');\n            }).catch(function() {});\n    }\n\n    window.appFilter = function(f) {\n        currentFilter = f;\n        ['all','pending','accepted','rejected'].forEach(function(s) {\n            var label = s === 'all' ? 'All' : s.charAt(0).toUpperCase() + s.slice(1);\n            var btn = document.getElementById('af' + label);\n            if (btn) btn.className = s === f\n                ? 'px-3 py-1.5 rounded-xl text-xs font-bold bg-purple-600 text-white transition shadow cursor-pointer'\n                : 'px-3 py-1.5 rounded-xl text-xs font-bold text-gray-400 hover:text-white hover:bg-white/5 transition cursor-pointer';\n        });\n        loadSubmissions(f);\n    };\n\n    window.reviewApp = function(id, status) {\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/applications/' + id + '/review', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: status }) })\n            .then(function(r) { return r.json(); })\n            .then(function(d) { if (d.success) loadSubmissions(currentFilter); })\n            .catch(function() {});\n    };\n\n    window.createApplication = function() {\n        var title = document.getElementById('appTitle').value.trim();\n        var desc = document.getElementById('appDesc').value.trim();\n        var qText = document.getElementById('appQuestions').value.trim();\n        var logCh = document.getElementById('appLogChannel').value;\n        var acceptRole = document.getElementById('appAcceptRole').value;\n        if (!title || !qText) return alert('يرجى إدخال العنوان والأسئلة على الأقل');\n        var questions = qText.split('\\n').filter(function(q) { return q.trim(); }).slice(0, 5);\n        var gId = window.location.pathname.split('/')[2];\n        fetch('/api/guild/' + gId + '/applications/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: title, description: desc, questions: questions, log_channel: logCh, accepted_role: acceptRole }) })\n            .then(function(r) { return r.json(); })\n            .then(function(d) { if (d.success) { alert('تم إنشاء التقديم بنجاح!'); document.getElementById('appTitle').value = ''; document.getElementById('appQuestions').value = ''; loadSubmissions('all'); } else alert('خطأ: ' + d.error); })\n            .catch(function() {});\n    };\n\n    loadSubmissions('all');\n})();\n</script>";
        } else if (section === 'fun') {
            formFieldsHtml = "﻿<div id=\"gamesRoot\" class=\"space-y-6 text-right\" dir=\"rtl\">\n\n    <!-- Header -->\n    <div class=\"bg-[#12141f] border border-white/5 p-6 rounded-2xl flex items-center justify-between shadow-xl\">\n        <div class=\"flex items-center gap-5\">\n            <div class=\"text-center\"><span id=\"gamesTotalPlayed\" class=\"text-2xl font-black text-purple-400 font-mono\">0</span><span class=\"text-[10px] text-gray-400 block\">جلسات لعب</span></div>\n            <div class=\"text-center\"><span id=\"gamesActiveGames\" class=\"text-2xl font-black text-emerald-400 font-mono\">0</span><span class=\"text-[10px] text-gray-400 block\">ألعاب مفعّلة</span></div>\n        </div>\n        <div class=\"flex items-center gap-3\">\n            <div class=\"text-right\">\n                <h4 class=\"font-black text-white text-base\">الألعاب والترفيه</h4>\n                <p class=\"text-gray-400 text-xs mt-0.5\">إدارة ألعاب السيرفر التفاعلية وإعداداتها</p>\n            </div>\n            <div class=\"w-11 h-11 rounded-xl bg-purple-600/20 text-purple-400 flex items-center justify-center text-xl border border-purple-500/30\">🎮</div>\n        </div>\n    </div>\n\n    <!-- Global Settings -->\n    <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl\">\n        <div class=\"flex items-center justify-end gap-2 mb-4\">\n            <h6 class=\"font-black text-white text-sm\">إعدادات عامة</h6>\n            <span>⚙️</span>\n        </div>\n        <div class=\"grid grid-cols-1 lg:grid-cols-3 gap-4\">\n            <div class=\"flex items-center justify-between bg-[#0b0d14] border border-white/5 rounded-xl px-4 py-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"funEnabled\" onchange=\"window.saveGameSetting('fun_enabled', this.checked)\" ${settings.fun_enabled ? 'checked' : ''}><span class=\"slider\"></span></label>\n                <div class=\"text-right\">\n                    <p class=\"text-xs font-bold text-white\">تفعيل الألعاب</p>\n                    <p class=\"text-[10px] text-gray-500\">تشغيل/إيقاف قسم الألعاب بالكامل</p>\n                </div>\n            </div>\n            <div class=\"flex items-center justify-between bg-[#0b0d14] border border-white/5 rounded-xl px-4 py-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"funRewards\" onchange=\"window.saveGameSetting('fun_rewards_enabled', this.checked)\" ${settings.fun_rewards_enabled ? 'checked' : ''}><span class=\"slider\"></span></label>\n                <div class=\"text-right\">\n                    <p class=\"text-xs font-bold text-white\">جوائز الألعاب</p>\n                    <p class=\"text-[10px] text-gray-500\">إعطاء عملات Gold عند الفوز</p>\n                </div>\n            </div>\n            <div class=\"bg-[#0b0d14] border border-white/5 rounded-xl px-4 py-3\">\n                <p class=\"text-xs font-bold text-white text-right mb-2\">قناة الألعاب المخصصة</p>\n                <select id=\"funChannel\" onchange=\"window.saveGameSetting('fun_channel', this.value)\" class=\"w-full bg-transparent text-xs text-white outline-none cursor-pointer\">\n                    <option value=\"\">أي قناة</option>\n                    ${guildChannels.filter(c=>c.type===0).map(c => `<option value=\"${c.id}\" ${settings.fun_channel === c.id ? 'selected' : ''}>💬 ${c.name}</option>`).join('')}\n                </select>\n            </div>\n        </div>\n    </div>\n\n    <!-- Games Grid -->\n    <div class=\"grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4\">\n\n        <!-- لعبة الكراسي الموسيقية -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_chairs\" onchange=\"window.toggleGame('chairs', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">الكراسي الموسيقية</p><p class=\"text-[10px] text-gray-500\">Musical Chairs</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-purple-950/40 border border-purple-800/20 flex items-center justify-center text-xl\">🪑</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">لعبة جماعية في قنوات الصوت — آخر متبقٍ في الكرسي يفوز!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-purple-950/40 text-purple-300 px-2 py-0.5 rounded\">صوتي</span>\n                <span class=\"bg-emerald-950/40 text-emerald-300 px-2 py-0.5 rounded\">جماعي</span>\n                <span class=\"bg-amber-950/40 text-amber-300 px-2 py-0.5 rounded\">جوائز</span>\n            </div>\n        </div>\n\n        <!-- مقلب العملة -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_coinflip\" onchange=\"window.toggleGame('coinflip', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">مقلب العملة</p><p class=\"text-[10px] text-gray-500\">Coin Flip</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-amber-950/40 border border-amber-800/20 flex items-center justify-center text-xl\">🪙</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">راهن على الصورة أو الكتابة وتحدى الأصدقاء!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-amber-950/40 text-amber-300 px-2 py-0.5 rounded\">مراهنة</span>\n                <span class=\"bg-emerald-950/40 text-emerald-300 px-2 py-0.5 rounded\">جوائز Gold</span>\n            </div>\n        </div>\n\n        <!-- المعركة -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_fight\" onchange=\"window.toggleGame('fight', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">المعركة</p><p class=\"text-[10px] text-gray-500\">Fight</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-rose-950/40 border border-rose-800/20 flex items-center justify-center text-xl\">⚔️</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">تحدى أي عضو في معركة نصية ممتعة!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-rose-950/40 text-rose-300 px-2 py-0.5 rounded\">تحدي</span>\n                <span class=\"bg-emerald-950/40 text-emerald-300 px-2 py-0.5 rounded\">جوائز</span>\n            </div>\n        </div>\n\n        <!-- الغميضة -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_hideseek\" onchange=\"window.toggleGame('hideseek', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">الغميضة</p><p class=\"text-[10px] text-gray-500\">Hide & Seek</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-blue-950/40 border border-blue-800/20 flex items-center justify-center text-xl\">🙈</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">لعبة الغميضة في القنوات الصوتية — اختبئ من الباحث!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-blue-950/40 text-blue-300 px-2 py-0.5 rounded\">صوتي</span>\n                <span class=\"bg-purple-950/40 text-purple-300 px-2 py-0.5 rounded\">جماعي</span>\n            </div>\n        </div>\n\n        <!-- المافيا -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_mafia\" onchange=\"window.toggleGame('mafia', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">المافيا</p><p class=\"text-[10px] text-gray-500\">Mafia</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-gray-900 border border-gray-700 flex items-center justify-center text-xl\">🕵️</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">لعبة المافيا الكلاسيكية — اكتشف المجرمين قبل فوات الأوان!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-gray-800 text-gray-300 px-2 py-0.5 rounded\">أدوار متعددة</span>\n                <span class=\"bg-purple-950/40 text-purple-300 px-2 py-0.5 rounded\">جماعي</span>\n            </div>\n        </div>\n\n        <!-- الروليت -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_roulette\" onchange=\"window.toggleGame('roulette', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">الروليت</p><p class=\"text-[10px] text-gray-500\">Roulette</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-red-950/40 border border-red-800/20 flex items-center justify-center text-xl\">🎰</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">راهن بعملاتك في لعبة الروليت المثيرة!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-red-950/40 text-red-300 px-2 py-0.5 rounded\">مراهنة</span>\n                <span class=\"bg-amber-950/40 text-amber-300 px-2 py-0.5 rounded\">Gold</span>\n            </div>\n        </div>\n\n        <!-- معلومة أو خرافة (Trivia) -->\n        <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl hover:border-purple-500/30 transition\">\n            <div class=\"flex items-center justify-between mb-3\">\n                <label class=\"toggle\"><input type=\"checkbox\" id=\"game_trivia\" onchange=\"window.toggleGame('trivia', this.checked)\" checked><span class=\"slider\"></span></label>\n                <div class=\"flex items-center gap-2\">\n                    <div class=\"text-right\"><p class=\"text-sm font-black text-white\">معلومة أو خرافة</p><p class=\"text-[10px] text-gray-500\">Trivia</p></div>\n                    <div class=\"w-10 h-10 rounded-xl bg-cyan-950/40 border border-cyan-800/20 flex items-center justify-center text-xl\">🧠</div>\n                </div>\n            </div>\n            <p class=\"text-[11px] text-gray-400 mb-3\">اختبر معلوماتك في أسئلة متنوعة وتنافس مع الأصدقاء!</p>\n            <div class=\"flex items-center gap-2 text-[10px] text-gray-500\">\n                <span class=\"bg-cyan-950/40 text-cyan-300 px-2 py-0.5 rounded\">ثقافي</span>\n                <span class=\"bg-emerald-950/40 text-emerald-300 px-2 py-0.5 rounded\">جوائز</span>\n            </div>\n        </div>\n\n    </div>\n\n    <!-- Rewards Settings -->\n    <div class=\"bg-[#12141f] border border-white/5 p-5 rounded-2xl shadow-xl\">\n        <div class=\"flex items-center justify-end gap-2 mb-4\">\n            <h6 class=\"font-black text-white text-sm\">إعدادات الجوائز</h6>\n            <span>🎁</span>\n        </div>\n        <div class=\"grid grid-cols-1 lg:grid-cols-3 gap-4\">\n            <div class=\"text-right\">\n                <label class=\"text-[11px] text-gray-400 block mb-1\">جائزة الفوز بالمعركة (Gold)</label>\n                <input type=\"number\" id=\"fightReward\" value=\"${settings.fight_reward || 50}\" onchange=\"window.saveGameSetting('fight_reward', this.value)\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none\">\n            </div>\n            <div class=\"text-right\">\n                <label class=\"text-[11px] text-gray-400 block mb-1\">جائزة الفوز بالكراسي (Gold)</label>\n                <input type=\"number\" id=\"chairsReward\" value=\"${settings.chairs_reward || 100}\" onchange=\"window.saveGameSetting('chairs_reward', this.value)\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none\">\n            </div>\n            <div class=\"text-right\">\n                <label class=\"text-[11px] text-gray-400 block mb-1\">جائزة Trivia صحيحة (Gold)</label>\n                <input type=\"number\" id=\"triviaReward\" value=\"${settings.trivia_reward || 25}\" onchange=\"window.saveGameSetting('trivia_reward', this.value)\" class=\"w-full bg-[#0b0d14] border border-white/5 focus:border-purple-500 rounded-xl px-3 py-2 text-xs text-white outline-none\">\n            </div>\n        </div>\n    </div>\n</div>\n\n<script>\n(function() {\n    window.toggleGame = function(game, enabled) {\n        window.saveGameSetting('game_' + game + '_enabled', enabled);\n        var count = document.querySelectorAll('.toggle input[type=\"checkbox\"][id^=\"game_\"]');\n        var active = 0;\n        count.forEach(function(cb) { if (cb.checked) active++; });\n        var el = document.getElementById('gamesActiveGames');\n        if (el) el.textContent = active;\n    };\n\n    window.saveGameSetting = function(key, value) {\n        var gId = window.location.pathname.split('/')[2];\n        var body = {}; body[key] = typeof value === 'boolean' ? (value ? 1 : 0) : value;\n        fetch('/api/guild/' + gId + '/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).catch(function() {});\n    };\n\n    // Count active games\n    var count = document.querySelectorAll('.toggle input[type=\"checkbox\"][id^=\"game_\"]');\n    var active = 0;\n    count.forEach(function(cb) { if (cb.checked) active++; });\n    var el = document.getElementById('gamesActiveGames');\n    if (el) el.textContent = active;\n})();\n</script>";
