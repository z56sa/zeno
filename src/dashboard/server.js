...
formFieldsHtml = `                    <div class="space-y-6 text-right" dir="rtl">
                ...
                ` : backupsList.map(b => {
                    ...
                    <div class="bg-[#12141f] border border-white/5 hover:border-purple-500/30 rounded-2xl p-4 flex items-center justify-between gap-4 transition">
                        <div class="flex items-center gap-2">
                            <button onclick="restoreBackup('${b.id}', this)" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold shadow transition">استعادة</button>
                            <button onclick="deleteBackup('${b.id}')" class="px-3 py-2 bg-rose-900/40 hover:bg-rose-700/50 text-rose-300 rounded-xl text-xs font-bold border border-rose-800/30 transition">🗑️</button>
                        </div>
                        <div class="flex-1 text-right">
...
formFieldsHtml = `<div class="space-y-6 text-right" dir="rtl">
    ...
    async function restoreBackup(id, btn) {
        if (!confirm('هل أنت متأكد من استعادة هذه النسخة الاحتياطية؟ سيتم إضافة العناصر المفقودة فقط.')) return;
