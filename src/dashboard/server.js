...
module.exports = function (app) {
    ...
    app.get('/', (req, res) => {
        try {
            ...
            <body class="min-h-screen flex flex-col justify-between purple-glow">
                ...
                ${hasError ? `
                    <div class="max-w-xl mx-auto mt-6 bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-xl text-xs font-bold text-center">
                        فشل تسجيل الدخول عبر Discord. يرجى التأكد من صحة بيانات التطبيق.
                    </div>
                ` : ''}
            ...
            res.status(500).send('Internal Server Error on Home');
        }
    });
    ...
    app.get('/auth/discord', (req, res) => {
        try {
            ...
            res.redirect('/?error=auth_redirect');
        }
    });
    ...
    app.get('/auth/discord/callback', async (req, res) => {
        ...
        try {
            ...
            // حفظ الجلسة صراحة قبل إعادة التوجيه لضمان تخزينها في قاعدة البيانات
